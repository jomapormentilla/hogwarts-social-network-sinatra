...
class ApplicationController < Sinatra::Base
    ...
    get '/search' do
        if params[:q]
            @houses = House.where("name like ?", "%#{ params[:q ]}%").includes(:wizards, :founder, :head_master)
            @wizards = Wizard.order(:name).where("name like ? or username like ?", "%#{ params[:q ]}%", "%#{ params[:q ]}%").includes(:friends, :added_friends, :wand, :house)
            @wands = Wand.where("name like ?", "%#{ params[:q ]}%").includes(:wizard)
            @spells = Spell.where("name like ?", "%#{ params[:q ]}%").includes(:wizards)
